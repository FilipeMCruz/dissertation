...
\begin{itemize}
    ...
    \item Providing various means to interact with the platform and the information handled by it, depending on the costumer needs, such as: (i) full-fledged access via \gls{UI}, (ii) high-level \gls{API} focussed on its core functionalities, (iii) low-level and generic \gls{API} to consume device measures and alerts.
\end{itemize}

To answer these high-level objectives the platform should encompass essencial functionalities such as:
...
Finally, this project requires the creation of \gls{PoC}s that answer specific business cases and utilize the platform developed. These \gls{PoC}s can follow distinct approaches for user interactions: from a full-fledged \gls{UI}, a simple and business case focussed \gls{API}, or a basic service that dispatches emails/SMS based on alerts captured.
...
During the third phase, a platform that comprises shared functionalities of all \gls{PoC}'s business cases must be designed and implemented.
...
This work gave birth to a platform, \textbf{Sensae Console}, capable of handling the desired requirements and functionalities. This platform proved it self capable of integrating with the \gls{IoT} middlewares researched while providing a semantic rich data model to consumers. The concepts tackled by this data model were materialized in a \gls{SDK}, \textit{iot-core}, that was developed to ease the integration with this platform's low-level and generic \gls{API}.

Three \gls{PoC}s were designed and implemented along this project duration: (i) fleet management, (ii) smart irrigation, (iii) notification management.
The platform and \gls{PoC}s were later evaluated according to the performance requirements envisioned for them.
