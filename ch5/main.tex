...
First the system scopes will be introduced to guide the reader through the system design. After this the system's architectural design will be presented and major decisions/alternatives discussed. Then a more detailed vision of the system is given by presenting the domain models of each scope's contexts. At last, a synopsis of this chapter can be read.

Scopes are derived from the major system responsibilities of the solution as a whole, contexts are derived from the major functionalities or business cases that the project has to answer.
...
A new C4 level, Context, that sits in between the System and Container level, and can be thought about as continental regions in a map (e.g. East Europe, Central Europe, North Europe) will be presented to the reader. This level is presented to ease the cognitive load required to envision the system as a single pile of containers.
...
The \textbf{External Services} are represented as an independent system that consumes the \textbf{Sensae Console} \gls{API}. This \gls{API} is responsible for streaming information such as device measures, device commands, alerts and internal state asynchronously.
