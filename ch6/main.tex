...
As we can see in Table~\ref{tab:evaluation:overview:decoproc:results}, the time taken to process or decode a data unit is very similar, this is possible due to \citetitle{graalvm}.
...
In both charts (Figures~\ref{fig:evaluation:overview:cache:chart:s2eB} and \ref{fig:evaluation:overview:cache:chart:s1eD}) the X axis represents time in seconds since the first request with a data unit was sent, the Y axis represents the time it took for the client to receive the corresponding measure. Each dot represents a data unit.
...
The \nameref{subsec:implementation:description:ingestion} only becomes a bottleneck with an enormous amount of devices for a small/medium organization.
